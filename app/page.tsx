    ...
    <div className="min-h-screen" style={{ background: "#0f0f0f", color: "#cfd2d8" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 pt-10 pb-8 grid md:grid-cols-2 gap-14 items-start">
        ...
        <div>
          ...
          >
            {[
              ...
              <div key={label} className="flex items-baseline gap-1.5">
                ...
                <span className="text-xs" style={{ color: "#64687a" }}>{label}</span>
              </div>
            ))}
          </div>

          {/* Who it's for */}
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "#33363e" }}>Para quem é</p>
          <div className="flex flex-wrap gap-2 mb-8">
            {personas.map(({ icon, label }) => (
              <span
                key={label}
                className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full"
                style={{ background: "rgba(255,255,255,0.04)", border: "1px solid #2a2d35", color: "#cfd2d8" }}
              >
          ...
          {/* What you'll build */}
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "#33363e" }}>O que você vai entregar</p>
            ...
            {deliverables.map(({ label, detail }) => (
              <div key={label} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: "#64687a" }}>{detail}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Acts */}
          <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "#33363e" }}>Estrutura do curso</p>
          <div className="space-y-2">
            {acts.map((act) => (
              ...
              >
                <span className="text-xs font-mono pt-0.5 shrink-0 w-14 whitespace-nowrap" style={{ color: (act as { gold?: boolean }).gold ? "rgba(209,164,118,0.4)" : "#33363e" }}>{act.n}</span>
                ...
                <span className="text-xs shrink-0 pt-0.5" style={{ color: (act as { gold?: boolean }).gold ? "rgba(209,164,118,0.4)" : "#33363e" }}>{act.time}</span>
              </div>
            ))}
          </div>
          ...
          >
            {status === "success" ? (
              ...
              <>
                ...
                <p className="text-sm mb-5" style={{ color: "#64687a" }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {[
                    ...
                    <div key={key}>
                      <label className="block text-xs font-medium mb-1.5" style={{ color: "#8b8f9a" }}>{label}</label>
                      <input
                        type={type}
                        required
                ...
                <div className="mt-5 pt-5 space-y-2" style={{ borderTop: "1px solid #1e2026" }}>
                  {[
                    ...
                    <p key={item} className="flex items-center gap-2 text-xs" style={{ color: "#64687a" }}>
                      <span style={{ color: "#4b6afc" }}>✓</span> {item}
                    </p>
                  ))}
                </div>
