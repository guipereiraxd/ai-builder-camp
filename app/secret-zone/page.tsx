    ...
    <AppShell>
      ...
      <Topic n={1} title="Nunca coloque chaves de API em arquivos de projeto" danger>
        <Risk>
          ...
          como <code>CLAUDE.md</code>, depois commitar para o GitHub. Bots varrem repositórios públicos
          em minutos e usam chaves expostas — você recebe uma fatura de milhares de dólares.
        </Risk>
        <Safe>
          Guarde chaves sempre em variáveis de ambiente (<code>ANTHROPIC_API_KEY</code>, etc.) e nunca
          diretamente em arquivos que vão para repositórios. Adicione <code>.env</code> e <code>.env.local</code>
          ao <code>.gitignore</code>.
        </Safe>
        <Rule>
      ...
      <Topic n={2} title="Dados sensíveis: o que vai para onde" danger>
        ...
        <Safe>
          Antes de usar IA com dados reais: leia a política de retenção do provedor. Anthropic e OpenAI
          oferecem planos sem retenção de dados para uso via API. Verifique se seu plano está configurado
          corretamente.
        </Safe>
        <Rule>
          Anonimize ou pseudonimize dados sensíveis antes de enviar para o modelo.
      ...
      <Topic n={6} title="Prompt injection: seu agente pode ser sequestrado">
        <p>
          ...
          Isso se chama <strong style={{ color: "var(--text-1)" }}>prompt injection</strong>.
        </p>
        <Risk>
      ...
      <Topic n={7} title="Auditoria: saiba o que seus agentes fizeram">
        ...
        <Rule>
          Instrua seus agentes a sempre salvar um log das ações executadas: arquivos criados,
          mensagens enviadas, dados acessados. Um arquivo <code>log-AAAA-MM-DD.md</code> por sessão
